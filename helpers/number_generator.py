...
def gen_sorted_integers():
    arr = [x for x in range(NUM)]
    cpp_array = "{"
    with open(file=f'{FILENAME_SORTED_INTEGERS}{END}', mode="w", encoding="UTF-8") as file:
        for num in arr:
            cpp_array += f'{str(num)},'
            file.write(str(num)+"\n")

    cpp_array = cpp_array[:-1] + "}"
    with open(file=f'{FILENAME_SORTED_INTEGERS_CPP}{END}', mode="w", encoding="UTF-8") as file:
        file.write(cpp_array)
